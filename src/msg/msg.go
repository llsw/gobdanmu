...
var (
	customHandlerMap map[string]func(string)
)

func init() {
	customHandlerMap = make(map[string]func(string))
	customHandlerMap[INTERACT_WORD] = OnInterActWord
	customHandlerMap[ENTRY_EFFECT] = OnEnterEffect
}
...
func GetEventHandler(eventName string) (string, func(s string)) {
	if f, ok := customHandlerMap[eventName]; ok {
		return eventName, func(s string) {
			go f(s)
		}
	} else {
		return eventName, func(s string) {
...
// 欢迎进入直播间
func OnWelcome(s string) {
}

func OnInterActWord(s string) {
	log.Infof("%s", s)
	data := gjson.Get(s, "data.uinfo.base.name").String()
	dmsg := NewDanMuMsg(
		"[欢迎]",
		fmt.Sprintf(" %s", data),
		"进入直播间",
